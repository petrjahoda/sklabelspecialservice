    ...
    public class Workplace {
        ...
        public bool CheckIfWorkplaceHasSpecialIdleOpened(ILogger logger) {
            ...
            try {
                connection.Open();
                var selectQuery =
                    $"SELECT * from zapsi2.terminal_input_idle where DTE is null and DeviceID={DeviceOid} and IdleID in (SELECT OID from zapsi2.idle where IdleTypeID IN (101,104) or OID in (10,11))";
                var command = new MySqlCommand(selectQuery, connection);
                try {
                    var reader = command.ExecuteReader();
                    if (reader.Read()) {
